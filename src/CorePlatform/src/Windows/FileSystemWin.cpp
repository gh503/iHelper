...
std::vector<uint8_t> FileSystem::ReadFile(const std::string& path) {
    ...
    // 添加长路径前缀（处理超过 MAX_PATH 的路径）
    if (wpath.size() >= MAX_PATH && wpath.substr(0, 4) != L"\\\\?\\") {
        wpath = L"\\\\?\\" + wpath;
    }
    ...
    HANDLE hFile = CreateFileW(
        wpath.c_str(),
        GENERIC_READ,
        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,  // 关键修改：允许更多共享
        NULL,
        OPEN_EXISTING,
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        ...
        throw std::system_error(
            error, 
            std::system_category(),
            "Failed to open file: " + path + " (Error: " + std::to_string(error) + ")"
        );
    }
    ...
    BOOL result = ::ReadFile(
        hFile,
        buffer.data(),
        static_cast<DWORD>(fileSize.QuadPart),  // 注意：32位系统可能截断
        &bytesRead,
        NULL
    );
    ...
    if (!result || bytesRead != static_cast<DWORD>(fileSize.QuadPart)) {
        throw std::system_error(
            ...
            "/" + std::to_string(fileSize.QuadPart) + " bytes)"
        );
    }
